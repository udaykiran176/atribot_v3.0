...
export default function Page() {
  ...
  const userCourses = SAMPLE_COURSES.filter(course => LEVELS[course.level].number <= levelInfo.number)
                ...
                <div className="flex flex-col md:flex-row items-center gap-6">
                  <div className="relative">
                    <div className="w-24 h-24 rounded-full bg-card border-4 border-primary/30 flex items-center justify-center">
                      <img src={(typeof atribotLogo === 'string' ? atribotLogo : (atribotLogo as any).src)} alt="Avatar" className="w-16 h-16" />
                    </div>
                  ...
                  <div className="flex-1 text-center md:text-left">
                    ...
                    <h1 className="font-display text-2xl md:text-3xl font-bold mb-2">Hello, {userName}! 👋</h1>
                    <LevelBadge level={userLevel} size="md" />
                    <p className="text-muted-foreground text-sm mt-2">{levelInfo.description}</p>
                  </div>
            ...
            <Card className="bg-card border-border">
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  <p className="text-sm text-muted-foreground">Complete more lessons to advance to the next level!</p>
                </div>
              </CardContent>
            </Card>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {userCourses.slice(0, 3).map((course, index) => (
                <motion.div key={course.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 + index * 0.1 }}>
                  <CourseCard {...course} progress={index === 0 ? 65 : index === 1 ? 30 : 0} />
                </motion.div>
              ))}
            </div>
          </motion.div>
