...
function Content() {
  ...
  const unlockedCourses = SAMPLE_COURSES.filter(course => LEVELS[course.level].number <= levelInfo.number)
      ...
      <main className="pt-24 pb-16">
        <div className="container mx-auto px-4">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
            <h1 className="font-display text-3xl md:text-4xl font-bold mb-2">Your Courses</h1>
            <p className="text-muted-foreground">Courses unlocked with your <span className="text-primary">{levelInfo.name}</span> kit</p>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="flex flex-col md:flex-row gap-4 mb-8">
            ...
            <div className="flex flex-wrap gap-2">
              <Button variant={selectedLevel === 'ALL' ? 'default' : 'outline'} size="sm" onClick={() => setSelectedLevel('ALL')}>All Levels</Button>
              {(Object.keys(LEVELS) as LevelId[]).map(levelId => {
                const level = LEVELS[levelId]
                const isUnlocked = level.number <= levelInfo.number
                return (
                  <Button key={levelId} variant={selectedLevel === levelId ? 'default' : 'outline'} size="sm" onClick={() => setSelectedLevel(levelId)} disabled={!isUnlocked} className={cn(!isUnlocked && 'opacity-50')}>
          ...
          {filteredCourses.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredCourses.map((course, index) => (
                <motion.div key={course.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 + index * 0.05 }}>
                  <CourseCard {...course} progress={index < 2 ? Math.floor(Math.random() * 70) + 10 : 0} />
                </motion.div>
              ))}
            </div>
          ) : (
