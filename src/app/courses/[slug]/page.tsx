...
import { Play, Clock, BookOpen, CheckCircle, ArrowLeft, Lock } from 'lucide-react'
        ...
        <main className="pt-24 pb-16">
          <div className="container mx-auto px-4">
            <div className="max-w-lg mx-auto text-center py-16">
              ...
              <p className="text-muted-foreground mb-6">This course requires Level {levelInfo.number} - {levelInfo.name}. Upgrade your kit to unlock it!</p>
              <Button asChild>
                <Link href="/courses">Back to Courses</Link>
              </Button>
            </div>
            ...
            <div className="lg:col-span-2">
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                <div className="relative aspect-video rounded-2xl overflow-hidden bg-gradient-to-br from-primary/20 to-secondary/20 mb-6 flex items-center justify-center">
                  <span className="text-8xl">{LEVELS[course.level].icon}</span>
                </div>
                ...
                <div className="flex flex-wrap gap-6 text-muted-foreground mb-8">
                  <div className="flex items-center gap-2"><BookOpen className="w-5 h-5" /><span>{lessons.length} lessons</span></div>
                ...
                <Card className="bg-card border-border">
                  <CardContent className="p-6">
                    ...
                    <ul className="space-y-3">
                      <li className="flex items-start gap-3"><CheckCircle className="w-5 h-5 text-secondary shrink-0 mt-0.5" /><span>Understand the fundamentals of robotics</span></li>
                      <li className="flex items-start gap-3"><CheckCircle className="w-5 h-5 text-secondary shrink-0 mt-0.5" /><span>Build hands-on projects with your kit</span></li>
                      <li className="flex items-start gap-3"><CheckCircle className="w-5 h-5 text-secondary shrink-0 mt-0.5" /><span>Learn problem-solving skills through robotics</span></li>
                      <li className="flex items-start gap-3"><CheckCircle className="w-5 h-5 text-secondary shrink-0 mt-0.5" /><span>Complete fun challenges and projects</span></li>
                    </ul>
                  </CardContent>
                </Card>
              </motion.div>
                  ...
                  <CardContent className="p-6">
                    ...
                    <div className="space-y-2">
                      {lessons.map((lesson, index) => {
                        ...
                        return (
                          <Link key={lesson.id} href={`/courses/${slug}/lesson/${lesson.id}`} className={cn('flex items-center gap-3 p-3 rounded-lg transition-colors', isCurrent && 'bg-primary/10 border border-primary/30', !isCurrent && 'hover:bg-muted')}>
                            <div className={cn('w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold', isCompleted && 'bg-secondary text-secondary-foreground', isCurrent && 'bg-primary text-primary-foreground', !isCompleted && !isCurrent && 'bg-muted text-muted-foreground')}>
                              {isCompleted ? (<CheckCircle className="w-4 h-4" />) : (index + 1)}
                            </div>
                            <div className="flex-1 min-w-0">
                              <p className={cn('font-medium text-sm truncate', isCompleted && 'text-muted-foreground')}>{lesson.title}</p>
                    ...
                    <Button variant="hero" className="w-full mt-6" asChild>
                      <Link href={`/courses/${slug}/lesson/1`}>
