...
import { Plus, Search, Copy, Download, QrCode, Trash2 } from 'lucide-react';
...
const LicenseKeys = () => {
  ...
  const handleGenerate = () => {
    ...
    toast({
      ...
      description: `${count} new ${LEVELS[generateLevel].name} key(s) created.`,
    });
  };
  ...
  const handleExportCSV = () => {
    const csv = [
      ...
      ...filteredKeys.map(k => 
        `${k.key},${LEVELS[k.level].name},${k.used ? 'Used' : 'Available'},${k.assignedUserId || ''},${k.createdAt}`
      )
    ].join('\n');
              ...
              <SelectContent>
                ...
                {(Object.keys(LEVELS) as LevelId[]).map(level => (
                  <SelectItem key={level} value={level}>
                    {LEVELS[level].icon} Level {LEVELS[level].number}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={generateLevel} onValueChange={(v) => setGenerateLevel(v as LevelId)}>
                      ...
                      <SelectContent>
                        {(Object.keys(LEVELS) as LevelId[]).map(level => (
                          <SelectItem key={level} value={level}>
                            {LEVELS[level].icon} Level {LEVELS[level].number} – {LEVELS[level].name}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                ...
                <TableBody>
                  {filteredKeys.map((key) => (
                    <TableRow key={key.id}>
                      ...
                      <TableCell>
                        <Badge className={LEVELS[key.level].color}>
                          {LEVELS[key.level].icon} Level {LEVELS[key.level].number}
                        </Badge>
                      </TableCell>
                      <TableCell>
            ...
            {qrKey && (
              <div className="text-center space-y-4 py-4">
                ...
                <div className="space-y-2">
                  ...
                  <Badge className={LEVELS[qrKey.level].color}>
                    {LEVELS[qrKey.level].icon} Level {LEVELS[qrKey.level].number} – {LEVELS[qrKey.level].name}
                  </Badge>
                </div>
